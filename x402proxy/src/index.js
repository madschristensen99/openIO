    ...
    app[methodLower](path, async (req, res) => {
      try {
        ...
        if (endpointConfig.forwardTo) {
          ...
          // Build proxy config
          const shouldForwardBody = ['POST', 'PUT', 'PATCH'].includes(method);
          const proxyConfig = {
            method: method,
            url: endpointConfig.forwardTo,
            validateStatus: () => true, // Don't throw on any status
            responseType: 'stream',
            data: shouldForwardBody ? req.body : undefined,
            params: req.query,
          };
          ...
          console.log(`Proxy config:`, {
            method,
            url: endpointConfig.forwardTo,
            hasBody: shouldForwardBody && !!req.body,
            headers: Object.keys(headersToForward),
          });
          ...
          if (forwardResponse.data && typeof forwardResponse.data.pipe === 'function') {
            console.log('Streaming upstream response to client');
            forwardResponse.data.on('error', (streamErr) => {
              console.error('Error while streaming upstream response:', streamErr.message);
              res.end();
            });
            forwardResponse.data.pipe(res);
          } else {
            // If axios didn't give us a stream, fall back to sending the buffered data
            console.log('Upstream response is not streamable, sending buffered data');
